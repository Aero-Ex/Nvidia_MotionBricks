...
class KeyboardHandler:
    ...
    def restore(self):
        if hasattr(self, 'old_settings') and hasattr(self, 'fd'):
            import termios
            try:
        ...
        if self._use_stdin:
            ...
            while select.select([sys.stdin], [], [], 0.0)[0]:
                try:
                    ...
                    if char:
                        self._stdin_last_pressed[char.lower()] = time.time()
                except Exception:
                    pass
            now = time.time()
            for key, ts in list(self._stdin_last_pressed.items()):
                if now - ts < 0.25:
                    keys.add(key)
                else:
                    self._stdin_last_pressed.pop(key, None)
        return keys
